        ...
        {    
           ...
           // auto plan_start = high_resolution_clock::now();
            ...
            if(count == 0 || startOver == 1)
                {
                    status = kAstar.search(startPose, startVel, startAcc, goalPose, goalVel, time_to_desination , true,  MMD_map_vis , MMD_map , A_star_vis ,   A_star_pub , false,   0.0);
                    startOver = 0;
                    ros::spinOnce();
                }                
            else
                {
                    status = kAstar.search(startPose, startVel, startAcc, goalPose, goalVel, time_to_desination,  false, MMD_map_vis , MMD_map , A_star_vis ,   A_star_pub  , false, 0.0 );
                    ros::spinOnce();
                }
            ...
            if(cTraj.size() > 2)
            {
            optimalTrajectory = optimizer.optimizeTrajectory(BernsteinTraj, cTraj, execTime, costMap3D , sample_trajectory_pub , plan_dur_pub);
