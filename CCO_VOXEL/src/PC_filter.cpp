...
void cloud_cb(const sensor_msgs::PointCloud2ConstPtr &cloud_msg)
{
  ...
  for (size_t points_i = 0; points_i < xyz_cloud->points.size(); ++points_i)
  {
    ...
    // float pointDepth2 = ( (xyz_cloud->points[points_i].x * xyz_cloud->points[points_i].x) +
    //                    (xyz_cloud->points[points_i].y * xyz_cloud->points[points_i].y) +
    //                    (xyz_cloud->points[points_i].z * xyz_cloud->points[points_i].z));

    float pointDepth2 = xyz_cloud->points[points_i].z;

    if (pointDepth2 < max_depth)
    {
...
{
  ROS_INFO(" PC filter code started");

  ros::init(argc, argv, "pcl_basics");
  ros::NodeHandle nh;
  nh.getParam("PC_filter/max_depth", max_depth);
